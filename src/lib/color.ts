...
// Feature (Transparent/neutral accent option): "Transparent" in the accent
// picker doesn't store a real color -- it stores this sentinel string, and
// every place that would normally receive the user's chosen accent color
// resolves it to the current theme's plain foreground color instead (white
// on dark, near-black on light). A LITERAL CSS `transparent` was
// considered and rejected: accentColor is used as a solid fill in dozens
// of places (Play button, Liked heart, badges, the Settings "Done"
// button), and `background: transparent` on those would make them
// invisible rather than just "uncolored". Resolving to the foreground
// color instead keeps every one of those elements visible and legible,
// just without any color tint -- a monochrome look, not a broken one.
export const NEUTRAL_ACCENT = 'neutral';

export function isNeutralAccent(value: string): boolean {
  return value === NEUTRAL_ACCENT;
}

/** Resolves a stored accent setting (a real hex, or the NEUTRAL_ACCENT
 *  sentinel) to an actual hex color safe to feed into every existing
 *  accentColor consumer (hex-alpha string concatenation, getContrastText,
 *  hexToHsv, etc). Matches this theme's --fg-rgb value in index.css. */
export function resolveAccentColor(value: string, theme: 'dark' | 'light'): string {
  if (value !== NEUTRAL_ACCENT) return value;
  return theme === 'light' ? '#15151a' : '#ffffff';
}
