...
// Feature (Library Overview stats): a composition-level summary of the
// library itself (how much music you have, not how much you've played) --
// distinct from every stat above, which is all about listening activity.
// Surfaced as its own card grid in StatsScreen.
export interface LibraryOverview {
  totalSongs: number;
  totalDurationMin: number;
  uniqueArtists: number;
  uniqueAlbums: number;
  /** File extension (lowercased, no dot) -> song count, sorted descending. */
  formatBreakdown: { format: string; count: number }[];
}

function extensionOf(fileName: string): string {
  const i = fileName.lastIndexOf('.');
  return i === -1 ? 'unknown' : fileName.slice(i + 1).toLowerCase();
}

export function computeLibraryOverview(songs: Song[]): LibraryOverview {
  ...
  for (const s of songs) {
    for (const name of splitArtists(s.artist)) artists.add(name);
    if (s.album) albums.add(s.album);
    totalDurationSec += s.duration || 0;
    const fmt = extensionOf(s.fileName);
...
// Feature (Forgotten Favorites): surfaces liked songs that haven't been
// played in a while (or ever) -- a "rediscover your library" nudge that's
// useful precisely because it's the opposite of every other stat here,
// which all reward whatever's already getting played the most.
export function computeForgottenFavorites(songs: Song[], likedIds: Set<string>, staleDays = 30, limit = 12): Song[] {
  const cutoff = Date.now() - staleDays * 86400000;
  return songs
    .filter((s) => likedIds.has(s.id) && (s.lastPlayedAt ?? 0) < cutoff)
    // Never-played (lastPlayedAt 0/undefined) first, then oldest last-played first.
