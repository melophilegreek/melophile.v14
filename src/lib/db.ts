...
export async function getPreferences(): Promise<Preferences> {
  const db = await getDB();
  const [color, crossfade, eq, sortBy, sortDir, playbackRate, preservePitch, autoTheme, osNotifications, rowSize, normalizeVolume] = await Promise.all([
    ...
    db.get('preferences', 'playbackRate'),
    db.get('preferences', 'preservePitch'),
    db.get('preferences', 'autoTheme'),
    db.get('preferences', 'osNotifications'),
    db.get('preferences', 'rowSize'),
    db.get('preferences', 'normalizeVolume'),
  ]);
  return {
    ...
    preservePitch: (preservePitch as boolean | undefined) ?? true,
    autoTheme: (autoTheme as boolean | undefined) ?? false,
    osNotifications: (osNotifications as boolean | undefined) ?? false,
    rowSize: (rowSize as Preferences['rowSize'] | undefined) ?? 'comfortable',
    normalizeVolume: (normalizeVolume as boolean | undefined) ?? false,
  };
}
