...
import { isNativePlayback, startPlaybackNotification, updatePlaybackNotification, stopPlaybackNotification } from './nativePlayback';
...
class Player {
  ...
  private _patch(patch: Partial<PlayerState>) {
    ...
    // Feature (Persistent playback notification): re-syncs on an actual
    // isPlaying flip (not on every single patch -- position updates alone
    // fire many times a second via timeupdate and would otherwise spam the
    // native side with redundant identical notification updates), and also
    // whenever currentSong itself is part of this patch.
    //
    // BUG FIX (stale notification/foreground-service left running forever):
    // that second condition used to be missing, so the only way this ever
    // re-ran was an isPlaying flip. That's fine for normal track changes
    // (loadSong's own _updateMediaSessionMetadata already calls
    // _syncNativePlaybackNotification directly once the new song is
    // loaded), but it meant the cases where currentSong goes straight to
    // null *without* an isPlaying flip -- deleting the one song that was
    // already paused, or clearAll() wiping the library while paused --
    // never called this at all. _syncNativePlaybackNotification's own
    // `if (!song)` branch (below) is what actually tears the notification
    // down in that case; it just needs to be reached.
    if ((patch.isPlaying !== undefined && patch.isPlaying !== wasPlaying) || 'currentSong' in patch) this._syncNativePlaybackNotification();
  ...
  private _syncNativePlaybackNotification() {
    ...
    const song = this._state.currentSong;
    if (!song) {
      // BUG FIX (stale notification/foreground-service left running
      // forever): previously this just returned here, leaving whatever
      // notification (and the foreground service backing it) was last
      // shown stuck on screen indefinitely -- e.g. deleting the very last
      // queued+playing song, or "Delete all songs" wiping the library
      // mid-playback. Tearing it down here, and resetting
      // _nativeNotificationStarted so a later song correctly calls
      // start() (not update()) again, mirrors what stop()/clearQueue()
      // already do for the web MediaSession case just above.
      if (this._nativeNotificationStarted) {
        this._nativeNotificationStarted = false;
