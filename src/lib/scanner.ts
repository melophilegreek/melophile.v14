...
// -- Folder-path metadata fallback --------------------------------------------
// Feature (Folder-path metadata inference): a lot of real-world files
// (especially older/re-encoded rips) have blank or garbage ID3/Vorbis/M4A
// tags but are still organized meaningfully on disk -- e.g.
// "A.R. Rahman/Roja/03 - Chinna Chinna Aasai.mp3". Previously a missing tag
// just fell straight to "Unknown Artist" / no album, discarding that folder
// structure entirely even though it often *is* the metadata. This only ever
// fills in gaps left by the real tags -- it never overrides an artist/album
// that was actually present in the file.
//
// Convention: last path segment (excluding the picked root folder itself,
// which is segments[0] -- see fsAccess.ts's collectFilesFromHandle for why
// every path is rooted there) is treated as the album, and the segment
// above that as the artist. A single folder directly under the root (e.g.
// "Downloads/Roja/track.mp3") is ambiguous between "this is the album" and
// "this is the artist" -- treated as album only, since one-folder-per-
// release is the more common convention than one-folder-per-artist with no
// album subfolder at all.
function inferFromFolderPath(folderPath: string): { artist?: string; album?: string } {
  if (!folderPath) return {};
  const segments = folderPath.split('/').filter(Boolean);
  const result: { artist?: string; album?: string } = {};
  if (segments.length >= 2) result.album = segments[segments.length - 1];
  if (segments.length >= 3) result.artist = segments[segments.length - 2];
  return result;
}
...
export async function importFiles(
...
): Promise<{ added: number; skipped: number; failed: { fileName: string; reason: string }[] }> {
  ...
  let completed = 0;
  let saved = 0;
  // Feature (Import failure report): every file that hits finalizeOne's
  // catch below used to just log to console.warn and vanish -- the calling
  // toast only ever showed a bare count ("3 files could not be imported")
  // with zero way to find out *which* files, let alone why. Collecting
  // {fileName, reason} here lets the UI actually show that list, and the
  // worker-crash fallback path further down pushes into the same array so
  // both failure sources end up in one place.
  ...
  // Perf fix (slow imports on large/mixed libraries): finalizeOne() does
  // real main-thread work per file -- folder-art lookup/read, an <audio>
  // element duration probe when the parser couldn't compute one from the
  // tag/header, and an .lrc sidecar read. Each worker's onmessage used to
  // `await finalizeOne(...)` *before* dispatching that worker's next file,
  // which meant the "parallel" worker pool was only ever as fast as this
  // single-threaded, partly I/O-bound step running one file at a time --
  // parsing on N workers doesn't help if only one file's worth of finalize
  // work can be in flight at once. Now dispatch happens immediately on
  // every parse result, and finalizeOne runs in the background; we collect
  // its promises here so importFiles() can still wait for every song to
  // actually be finalized (and therefore queued for a DB write) before it
  // resolves.
  ...
  const finalizeOne = async (file: File, meta: Meta) => {
    try {
      ...
      // Feature (Folder-path metadata inference): only consulted for
      // whichever of artist/album the real tags left empty -- see the
      // comment on inferFromFolderPath above.
      const folderInferred = (!meta.artist || !meta.album) ? inferFromFolderPath(folderOf(file)) : {};
      const artist = meta.artist || folderInferred.artist || 'Unknown Artist';
      const album = meta.album || folderInferred.album;
      ...
      const song: Song = {
        ...
        // Feature (Genre/Year/Track#): now actually extracted at import
        // time (TCON/TYER/TDRC/TRCK for MP3, Vorbis comments for FLAC/OGG/
        // Opus, '\xa9gen'/'gnre'/'\xa9day'/'trkn' for M4A) instead of being
        // purely user-entered via Edit Tags.
        genre: meta.genre, year: meta.year, trackNumber: meta.trackNumber,
      };
      ...
      failed.push({ fileName: file.name, reason: e instanceof Error ? e.message : 'Unknown error' });
    } finally {
      completed++;
  ...
  await new Promise<void>((resolve) => {
    ...
    for (const worker of workers) {
      worker.onmessage = (e: MessageEvent<WorkerResponse>) => {
        ...
        // Dispatch this worker's next file immediately -- don't wait on
        // finalizeOne (see finalizePromises comment above). Its promise is
        // collected, not awaited, so parsing and finalizing overlap.
        remaining--;
        settleIfDone();
        if (remaining > 0) dispatchNext(worker);
        finalizePromises.push(finalizeOne(audioFiles[id], meta));
      };
      worker.onerror = () => {
        ...
        // Same overlap fix applies here: don't let the fallback parse+
        // finalize block this worker slot from moving on.
        ...
        if (remaining > 0) dispatchNext(worker);
        if (file) {
          finalizePromises.push((async () => {
            try {
              const { extractMeta } = await import('./metadataParser');
              const meta = await extractMeta(file);
              await finalizeOne(file, meta);
            } catch (e) {
              // FIX: this fallback used to run un-guarded -- if the retry
              // itself threw (rather than finalizeOne's own try/catch
              // catching it), it became an unhandled promise rejection and
              // the file silently never got counted as failed at all.
              console.warn('Failed to import (worker fallback)', file.name, e);
              failed.push({ fileName: file.name, reason: e instanceof Error ? e.message : 'Worker crashed' });
              completed++;
              onProgress({ current: completed, total, fileName: file.name });
            }
          })());
  ...
  // All files have been dispatched to and returned from the worker pool,
  // but finalizeOne() calls triggered by the last few onmessage events may
  // still be running in the background (that's the whole point of the
  // overlap fix above) -- wait for those to actually finish so every song
  // has been pushed into pendingBatch before we flush.
  ...
  return { added, skipped: total - added, failed };
}
