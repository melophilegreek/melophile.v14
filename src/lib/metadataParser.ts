...
export interface Meta { title?: string; artist?: string; album?: string; duration?: number; kbps?: number; artData?: ArrayBuffer; artMime?: string; lyrics?: string; genre?: string; trackNumber?: number; year?: number; }

// Feature (Genre/Year/Track#): the standard ID3v1 genre list. TCON (ID3v2)
// and MP4's numeric 'gnre' atom can both reference a genre purely by this
// index instead of spelling it out as text, so it's needed for every format
// below, not just ID3v1 itself. Stops at the original 80 (0-79) plus the
// widely-supported Winamp extension through 125 -- genre IDs past that
// point were never consistently assigned across taggers, so guessing at
// them would risk showing a *wrong* genre rather than none at all.
...
// TCON's legacy format wraps a numeric ID3v1 genre reference in parens --
// "(17)" alone, or "(17)Trance" where the trailing text is a free-text
// *refinement* some taggers add on top of the numeric one. v2.4 also allows
// chaining several of these, e.g. "(4)(24)". We only need one genre string
// to show the user, so: prefer trailing free text if present, otherwise
// resolve the first numeric ref through the table above, otherwise (no
// parens at all) the value is already plain text and used as-is.
function resolveGenre(raw: string): string | undefined {
  ...
  if (m) {
    ...
    return ID3V1_GENRES[n] || undefined;
  }
  return s;
}

// TYER (v2.3) is a plain 4-digit year. TDRC (v2.4) is a full ISO-8601-ish
// timestamp ("2019", "2019-05", or "2019-05-17T00:00:00") of which only the
// leading 4 digits are needed. Vorbis DATE and MP4 ©day share the same
// "just take the first 4-digit run" shape, so this is reused by every
// format below.
function resolveYear(raw: string): number | undefined {
  const m = /\d{4}/.exec(raw); if (!m) return undefined;
  const y = parseInt(m[0], 10);
  return y > 0 ? y : undefined;
}

// TRCK ("5" or "5/12"), Vorbis TRACKNUMBER (same shapes) -- total-track
// count after the slash, if any, isn't tracked by the Song model, so only
// the leading number is kept.
function resolveTrackNumber(raw: string): number | undefined {
  const m = /^\s*(\d+)/.exec(raw); if (!m) return undefined;
  const n = parseInt(m[1], 10);
...
function parseID3v2(buf: Uint8Array): Meta {
  ...
  while(pos+hdr<end) {
    ...
    // Feature (Genre/Year/Track#): TCON/TCO = genre, TYER/TYE (v2.3) or
    // TDRC (v2.4 timestamp) = year, TRCK/TRK = track number. All are plain
    // text frames like TIT2/TPE1 above, so `rt()` handles the encoding.
    else if(id==='TCON'||id==='TCO'){ const g=resolveGenre(rt()); if(g) meta.genre=g; }
    else if(id==='TYER'||id==='TYE'||id==='TDRC'){ const y=resolveYear(rt()); if(y) meta.year=y; }
    else if(id==='TRCK'||id==='TRK'){ const n=resolveTrackNumber(rt()); if(n) meta.trackNumber=n; }
...
function parseID3v1(buf: Uint8Array): Meta {
  ...
  const meta: Meta = {title:lstr(t,3,30)||undefined,artist:lstr(t,33,30)||undefined,album:lstr(t,63,30)||undefined};
  const genre = ID3V1_GENRES[t[127]]; if(genre) meta.genre=genre;
  const year = resolveYear(lstr(t,93,4)); if(year) meta.year=year;
  // ID3v1.1 extension: repurposes the last two bytes of the (otherwise
  // 30-byte) comment field for a track number -- a zero byte at offset 125
  // (impossible in a real comment, which would need all 28 preceding bytes
  // used) signals "this is v1.1, byte 126 is the track number" rather than
  // just two more comment characters.
  if(t[125]===0 && t[126]>0) meta.trackNumber=t[126];
  return meta;
}
...
function parseVorbis(buf: Uint8Array, o: number): Meta {
  ...
  try{
    ...
    for(let i=0;i<cc&&p<buf.length;i++){
      ...
      // Feature (Genre/Year/Track#): Vorbis comments have no fixed field
      // list, but these are the field names every common tagger (foobar2000,
      // Picard, etc) actually writes for genre/year/track.
      else if(k==='genre'){ const g=resolveGenre(v); if(g) meta.genre=g; }
      else if(k==='date'||k==='year'){ const y=resolveYear(v); if(y) meta.year=y; }
      else if(k==='tracknumber'){ const n=resolveTrackNumber(v); if(n) meta.trackNumber=n; }
...
function parseM4a(buf: Uint8Array, fileSize: number): Meta {
  ...
  // Feature (Genre/Year/Track#): unlike the text atoms above (flags 1/13/14
  // = UTF-8/JPEG/PNG), 'gnre' and 'trkn' store their value as raw binary
  // (flags 0, "reserved") regardless of what it semantically represents, so
  // `rda()`'s text-flag check would always reject them -- this pulls the
  // 'data' atom's payload bytes with no flag/encoding assumption.
  const rdRaw=(parent:Uint8Array)=>{const d=find(parent,'data');if(!d)return null;return parent.slice(d.s+16,d.s+d.sz);};
  // Feature (Lyrics): '\xa9lyr' is the standard iTunes/MP4 lyrics atom,
  // same text-data-atom shape as name/artist/album.
  const tags:Record<string,string>={'\xa9nam':'t','\xa9ART':'a','\xa9alb':'b',covr:'c','\xa9lyr':'l','\xa9gen':'g','gnre':'gn','\xa9day':'y',trkn:'tr'};
  let p=0;while(p+8<=id.length){const sz=u32be(id,p);if(sz<8)break;
    ...
    // '\xa9gen' is free-text genre (iTunes writes this for anything not in
    // its own preset list); 'gnre' is the older numeric form -- a 1-based
    // index (unlike ID3's 0-based TCON numeric form) into the same ID3v1
    // genre table, stored as a big-endian uint16.
    else if(f==='g'){const g=resolveGenre(rda(atom));if(g)meta.genre=g;}
    else if(f==='gn'){const raw=rdRaw(atom);if(raw&&raw.length>=2){const genre=ID3V1_GENRES[u16be(raw,0)-1];if(genre)meta.genre=genre;}}
    else if(f==='y'){const y=resolveYear(rda(atom));if(y)meta.year=y;}
    // 'trkn' payload: 8 bytes, [reserved(2)][track(2)][total(2)][reserved(2)].
    else if(f==='tr'){const raw=rdRaw(atom);if(raw&&raw.length>=4){const n=u16be(raw,2);if(n>0)meta.trackNumber=n;}}
    p+=sz;
  }
  return meta;
}
