...
function Card({ label, value, accentColor }: { label: string; value: string; accentColor: string }) {
  return (
    <div
      className="rounded-2xl p-3 transition-colors hover:bg-fg/[0.06]"
      style={{ background: 'rgb(var(--fg-rgb) / 0.04)', border: '1px solid rgb(var(--fg-rgb) / 0.06)' }}
    >
      <span className="text-fg/40 text-[11px] font-medium uppercase tracking-wider block mb-1.5">{label}</span>
