...
export function Onboarding({ accentColor, onComplete }: Props) {
  ...
  // folder-picker swap as App.tsx's import handlers -- see
  // lib/nativeFolderImport.ts's header comment.
  const handleFolderButtonClick = async () => {
    if (isNativeFolderPickerAvailable()) {
      setProgress({ current: 0, total: 0, fileName: 'Scanning folder…' });
      try {
        const result = await pickNativeFolder((found) => setProgress({ current: found, total: found, fileName: 'Scanning folder…' }));
        if (!result) return; // cancelled
        if (result.files.length === 0) { setProgress(null); setDone({ added: 0, skipped: 0 }); return; }
  ...
  return (
    <div className="h-full w-full flex flex-col items-center justify-center bg-bg px-6">
      <div className="w-full max-w-md text-center animate-fade-in">
        ...
        <h1 className="text-3xl font-bold text-fg mb-2">Melophile</h1>
        <p className="text-fg/45 mb-10">Your personal offline music player</p>
        ...
        {progress ? (
            ...
            <p className="text-fg/60 text-sm">
              {progress.finalizing ? `Saving ${progress.current} / ${progress.total}…` : (
                <>
                  ...
                  {progress.fileName && <span className="text-fg/35"> — {progress.fileName}</span>}
                </>
              )}
            </p>
          ...
          <div className="animate-fade-in space-y-4">
            {done.added > 0 ? (
              <div className="rounded-xl p-4 border" style={{ background: `${accentColor}15`, borderColor: `${accentColor}40` }}>
                ...
                <p className="text-fg/35 text-xs mt-1">Opening your library…</p>
              </div>
            ) : (
            ...
            <button onClick={handleFolderButtonClick}
              ...
              style={{ background: accentColor, color: getContrastText(accentColor), boxShadow: `0 8px 24px -6px ${accentColor}80` }}>
          ...
          <div className="space-y-3">
            ...
            <button onClick={() => fileInputRef.current?.click()}
              className="w-full flex items-center justify-center gap-2 font-medium py-3 rounded-full bg-fg/5 hover:bg-fg/10 text-fg/70 transition-colors text-sm">
              <Music size={18} /> Select individual files
            </button>
          </div>
        )}
        <p className="text-fg/20 text-xs mt-6">Music never leaves your device · Everything stays offline</p>
      </div>
    </div>
  );
}
