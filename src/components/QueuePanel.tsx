    ...
    <div ref={ref} className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 shrink-0"
        ...
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-fg font-bold text-lg">Queue</h2>
          {queue.length > 0 && (
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={onClose} className="btn-icon w-8 h-8 hover:bg-fg/10 rounded-lg">
      ...
      {currentSong && (
        <div className="px-4 py-3 shrink-0">
          <p className="text-xs text-fg/30 uppercase tracking-wider font-semibold mb-2">Now Playing</p>
      ...
      {/* Queue list */}
      {queue.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center py-16 text-fg/25">
          <ListMusic size={40} className="mb-3 text-fg/15" />
        ...
        <VirtualList
          ...
          renderItem={(row) => {
            if (row.kind === 'header') {
              return <p className="text-xs text-fg/30 uppercase tracking-wider font-semibold mb-2 mt-2">Next Up</p>;
            }
...
function QueueRow({ song, isCurrent, isPlaying, accentColor, onPlay, onRemove, onMoveUp, onMoveDown, onGripPointerDown, onQueueSong, isDragging, isDropTarget, index, isAutoQueued }: {
  ...
  return (
    <div className="relative overflow-hidden rounded-lg" style={{
      ...
      <div
        className="group flex items-center gap-2 py-2 rounded-lg cursor-pointer hover:bg-fg/5 transition-colors px-2 -mx-2"
        style={{
      ...
      >
        ...
        <div className="w-5 shrink-0 flex items-center justify-center">
          {index !== undefined && !isCurrent && !isAutoQueued && (
            <div onPointerDown={onGripPointerDown} style={{ touchAction: 'none', cursor: 'grab' }} className="p-1 -m-1">
              <GripVertical size={14} className="text-fg/20 group-hover:text-fg/40 transition-colors" />
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium truncate" style={{ color: isCurrent ? accentColor : 'rgb(var(--fg-rgb) / 0.9)' }}>
            {song.title}
          </p>
          <p className="text-xs text-fg/40 truncate">{song.artist}</p>
        </div>
        ...
        {!isCurrent && (onMoveUp || onMoveDown) && (
          <div className="flex flex-col shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
            {onMoveUp && (
              <button onClick={(e) => { e.stopPropagation(); onMoveUp(); }} className="w-5 h-4 flex items-center justify-center text-fg/40 hover:text-fg">
                <ChevronUp size={12} />
              </button>
            )}
            {onMoveDown && (
              <button onClick={(e) => { e.stopPropagation(); onMoveDown(); }} className="w-5 h-4 flex items-center justify-center text-fg/40 hover:text-fg">
                <ChevronDown size={12} />
              </button>
            )}
          </div>
