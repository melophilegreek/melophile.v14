...
export function Slider({ value, min, max, step = 1, accentColor, onChange, ariaLabel, className }: SliderProps) {
  ...
  return (
    ...
    >
      {/* Track */}
      <div className="absolute left-0 right-0 h-1.5 rounded-full bg-fg/10" />
      {/* Zero tick, bipolar ranges only */}
      {min < 0 && max > 0 && (
        <div className="absolute w-px h-3 bg-fg/20" style={{ left: `${zeroPct}%` }} />
      )}
      {/* Fill */}
      ...
      <div
        ...
        style={{
          ...
          // Feature (Premium light mode pass, round 2): was a hardcoded
          // dark-only drop shadow -- fine on dark mode's near-black track,
          // but the same heavy black shadow reads as a dark smudge under a
          // small light-mode thumb instead of a soft lift. var(--shadow-lift)
          // carries the theme-appropriate recipe (see index.css).
          boxShadow: dragging ? 'var(--shadow-lift)' : '0 1px 3px rgb(21 21 26 / 0.25)',
